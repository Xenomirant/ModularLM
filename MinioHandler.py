...
class MinioHandler(object):
    ...
    def __init__(self):
        '''
        Don't forget to setup config file for minio with necessary credentials
        '''

        self.minio = Minio(config.Minio_ip, 
    ...
    def put_object(self, obj: object, *, save_name: str, dir: str = "ModularLM/", bucket: str = "public", pickle: bool = False):
        ...
        if pickle:
            obj = pkl.dumps(obj)
        object_len = len(obj)
        pkl_buffer = io.BytesIO(obj)
            
        
        resp = self.minio.put_object(bucket_name = bucket,
                            object_name = dir+save_name,
                            data = pkl_buffer,
                            length = object_len,
                            progress = Progress(),
                            )
    ...
    def get_object(self, object_name: str, *, dir: str = "ModularLM/", bucket: str = "public", type: Optional[str] = None, unpickle: bool = False):
        ...
        '''
        try:
            res = self.minio.get_object(bucket, dir+object_name)
            obj = res.data
        finally:
            res.close()
            res.release_conn()
