...
# from attribution import compute_primary_attributions_scores
...
class LM(object):
    ...
    def _analyze_token(self,
        ...
        for attr_method in attribution_flags:
            ...
            if poly_flag is not None: 
                forward_kwargs['poly_flag']=poly_flag.to(self.device).tile(50)
    ...
    def generate(self, input_str: str,
        ...
        # for mem_id in output.sequences[0][:2]:
    ...
    def __call__(self, input_tokens: torch.Tensor, attribution: Optional[List[str]] = []):
        """
        ...
        if self.model_type == 'mlm':
            # bert_out = self.model.bert()
            output = self.model(**input_tokens, return_dict=True, output_hidden_states=True)
            # mem_outs = 
            lm_head = self.model.cls
            # lm_head = self.model.head
        ...
        encoder_input_embeds, _ = self._get_embeddings(input_tokens['input_ids'])
        prediction_ids = torch.argmax(output.logits[0], axis=-1)
        attention_mask = input_tokens['attention_mask'].tile(50,1)


        for i in range(64-(input_tokens['input_ids']==0).sum().item()):
            prediction_id = prediction_ids[i]
            cur_encoder_input_embeds = encoder_input_embeds[:,:,:]
            # cur
            self._analyze_token(
                encoder_input_embeds=cur_encoder_input_embeds,
                encoder_attention_mask=attention_mask,
                decoder_input_embeds=None,
                attribution_flags=attribution,
                prediction_id=prediction_id,
                # poly_flag = input_tokens.get('poly_flag', None)
            )

        # Turn activations from dict to a proper array
        activations_dict = self._all_activations_dict
        # activations_dict['bert.embeddings'] = self.mode
        for layer_type, activations in activations_dict.items():
            self.activations[layer_type] = activations_dict_to_array(activations)
        ...
        # attributions=None
        attributions = self.attributions
        if tokens[0][-1] in ['[PAD]','[SEP]']:
            for att, vals in attributions.items():
                for ar in vals:
                    ar[-1] = 0
                    ar = ar / sum(ar)
        return OutputSeq(**{'tokenizer': self.tokenizer,
    ...
    def _inhibit_neurons_hook(self, name: str, input_tensor):
        ...
        """
        layer_number = int(re.findall('\d+', name)[0])
        # try:
        #     layer_number = int(name.split('.')[2])
        # except ValueError:
